    ...
    <section className="py-24 bg-[#f8faff]">
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-display font-bold text-[#1a2b3c] mb-12">Career Journey</h2>
            ...
            {timelineData.map((item, index) => (
              <div key={index} className={`relative group ${item.isWonderGroup ? 'ml-6' : ''}`}>
                <div className={`absolute left-0 top-0 w-px h-full 
                  ${item.isWonderGroup 
                    ? 'bg-gradient-to-b from-[#1EAEDB] to-[#0FA0CE] shadow-lg shadow-[#1EAEDB]/20' 
                    : 'bg-gray-200'} 
                  group-last:h-24`}
                ></div>
                ...
                {item.isWonderGroup && (
                  <div className="absolute -left-6 top-1/2 -translate-y-1/2">
                    <ChevronRight className="w-4 h-4 text-[#1EAEDB]" />
                  </div>
                )}
                ...
                <div className="relative pl-8 transition-all">
                  <div className={`absolute left-0 top-2 -translate-x-1/2 w-4 h-4 rounded-full border-2 
                    ${item.isWonderGroup 
                      ? 'border-[#1EAEDB] bg-[#1EAEDB]/20 shadow-lg shadow-[#1EAEDB]/30 ring-2 ring-[#1EAEDB]/20 ring-offset-2 ring-offset-[#f8faff]' 
                      : 'border-[#1EAEDB] bg-white'}`}>
                  </div>
                  
                  <div className={`${item.isWonderGroup 
                    ? 'bg-[#e6f7ff] border border-[#1EAEDB]/20 relative after:absolute after:inset-0 after:bg-gradient-to-r after:from-[#1EAEDB]/5 after:to-transparent after:opacity-50' 
                    : 'bg-white shadow-sm'} 
                    backdrop-blur-sm p-6 rounded-lg 
                    ${item.isWonderGroup ? 'hover:bg-[#d9f2ff]' : 'hover:bg-gray-50'} 
                    transition-all overflow-hidden`}>
                    <div className="text-[#1EAEDB] text-sm font-medium mb-2">{item.period}</div>
                    <h3 className="text-xl font-display text-[#222222] mb-1 relative z-10">{item.role}</h3>
                    <div className="text-[#555555] text-sm mb-4 relative z-10">{item.company}</div>
                    <p className="text-[#333333] relative z-10">{item.description}</p>
                    
                    <button className="mt-4 text-[#1EAEDB] hover:text-[#0FA0CE] inline-flex items-center text-sm font-medium transition-colors relative z-10">
